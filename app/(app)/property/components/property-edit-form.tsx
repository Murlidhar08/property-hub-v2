...
export function PropertyEditForm({
    ...
    const { register, control, handleSubmit, watch, setValue, formState: { errors, isSubmitting } } = useForm<any>({
        ...
        defaultValues: {
            ...
            propertyType: initialData.propertyType || "agricultural",
            address: initialData.address || "",
            statusDetails: initialData.status?.length > 0
                ? initialData.status.map((s: any) => ({
                    status: s.status,
                    price: s.price?.toString() || "",
                    durationTypeId: s.durationTypeId || null
                }))
                : [{ status: "sell" }],
            isUnknownMeasurement: !initialData.measurementValue,
            isPriceUnknown: initialData.status?.every((s: any) => !s.price),
            isTotalPrice: false, // Assuming default false or check logic if available
            measurementValue: initialData.measurementValue?.toString() || "",
            measurementType: initialData.measurementType || "acer",
            owners: initialData.users?.filter((u: any) => u.userType === 2).map((u: any) => u.userId) || [],
            agents: initialData.users?.filter((u: any) => u.userType === 0).map((u: any) => u.userId) || [],
            hasOwner: initialData.users?.some((u: any) => u.userType === 2) || false,
            hasAgent: initialData.users?.some((u: any) => u.userType === 0) || false,
            description: initialData.description || "",
            hasLocation: !!initialData.coordinates,
            mapDetails: (initialData.coordinates as any) || { lat: 23.0225, lng: 72.5714, zoom: 12 },
            images: initialData.documents?.filter((d: any) => d.documentType === "preview") || [],
            documents: initialData.documents?.filter((d: any) => d.documentType === "document") || [],
        }
    });
    ...
    const toggleStatus = (status: PropertyStatus) => {
        const index = currentStatuses.findIndex((s: any) => (s as any).status === status);
        if (index > -1) {
    ...
    const onSubmit: SubmitHandler<PropertyFormValues> = async (values) => {
        try {
            ...
            const property = await updateProperty(propertyId, submitData);

            // ⚡ Handle newly added documents (the ones without IDs)
            const newDocs = [
                ...values.images,
                ...values.documents
            ].filter(d => !d.id);
            ...
            if (newDocs.length > 0) {
                await finalizePropertyDocuments(propertyId, newDocs);
            }
    ...
    return (
        <form onSubmit={handleSubmit(onSubmit)} className="mx-auto max-w-4xl mt-6 space-y-8 px-6">
                ...
                <Card className="p-6 rounded-[2rem] border-border bg-card shadow-sm">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div className="space-y-2">
                            <Label>Type</Label>
                            <Select
                                onValueChange={(val) => setValue("propertyType", val as PropertyType)}
                                defaultValue={initialData.propertyType || "agricultural"}
                            >
                ...
                <Card className="p-6 rounded-[2rem] border-border bg-card shadow-sm">
                    <div className="flex justify-between items-center mb-6">
                        ...
                        <div className="flex items-center space-x-2">
                            <Checkbox
                                id="isUnknownMeasurement"
                                checked={isUnknownMeasurement}
                                onCheckedChange={(val) => setValue("isUnknownMeasurement", !!val)}
                            />
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div className="space-y-2">
                            ...
                            <Select
                                ...
                                defaultValue={initialData.measurementType || "acer"}
                            >
                                <SelectTrigger className="rounded-xl h-12 w-full">
                                    <SelectValue placeholder="Select type">
                        ...
                        <div className="flex gap-6">
                            <div className="flex items-center space-x-2">
                                <Checkbox
                                    ...
                                    onCheckedChange={(val) => setValue("isPriceUnknown", !!val)}
                                />
                            ...
                            <div className="flex items-center space-x-2">
                                <Checkbox
                                    ...
                                    onCheckedChange={(val) => setValue("isTotalPrice", !!val)}
                                />
                        ...
                        {fields.map((field: any, index) => (
                            <div key={field.id} className="grid grid-cols-1 md:grid-cols-3 gap-6 p-4 rounded-3xl bg-muted/30 relative group">
                                <div className="space-y-2 text-wrap">
                                    <Label className="capitalize">{field.status}</Label>
                                    <div className="relative">
                                        <Input
                                ...
                                {field.status === "rent" && (
                                    <div className="space-y-2">
                                        ...
                                        <Select
                                            ...
                                            defaultValue={field.durationTypeId || "month"}
                                        >
                                            <SelectTrigger className="rounded-xl h-12 w-full">
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }}>
                <Card className="p-6 rounded-[2rem] border-border bg-card shadow-sm">
                    ...
                    <div className="space-y-8">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-end">
                            <div className="flex items-center space-x-2 pb-3">
                                <Checkbox
                                    id="hasOwner"
                                    checked={hasOwner}
                                    onCheckedChange={(val) => setValue("hasOwner", !!val)}
                                />
                        ...
                        {/* Owners List */}
                        {hasOwner && watch("owners")?.length > 0 && (
                            <div className="flex flex-wrap gap-2">
                                {watch("owners").map((id: string) => {
                                    const owner = allOwners.find(o => o.id === id);
                                    return (
                                        <div key={id} className="flex items-center gap-2 bg-muted px-3 py-1 rounded-full text-sm">
                                            {owner?.name || id}
                                            <button type="button" onClick={() => setValue("owners", watch("owners").filter((o: string) => o !== id))}>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-end">
                            <div className="flex items-center space-x-2 pb-3">
                                <Checkbox
                                    id="hasAgent"
                                    checked={hasAgent}
                                    onCheckedChange={(val) => setValue("hasAgent", !!val)}
                                />
                        ...
                        {/* Agents List */}
                        {hasAgent && watch("agents")?.length > 0 && (
                            <div className="flex flex-wrap gap-2">
                                {watch("agents").map((id: string) => {
                                    const agent = allAgents.find(a => a.id === id);
                                    return (
                                        <div key={id} className="flex items-center gap-2 bg-muted px-3 py-1 rounded-full text-sm">
                                            {agent?.name || id}
                                            <button type="button" onClick={() => setValue("agents", watch("agents").filter((a: string) => a !== id))}>
                                                <Trash2 size={12} className="text-destructive" />
                                            </button>
                                        </div>
                                    );
                ...
                <div className="grid grid-cols-1 gap-8">
                    ...
                    <Card className="p-8 rounded-[2.5rem] border-border bg-card shadow-sm overflow-hidden relative">
                        ...
                        <PropertyImageUploadZone
                            ...
                            initialFiles={watch("images") as any}
                            onFilesChange={(files: any[]) => {
                                const mapped = files.map((f: any, idx: number) => ({
                                    ...f,
                                    documentType: "preview" as PropertyDocumentType,
                                    orderId: idx,
                    ...
                    <Card className="p-8 rounded-[2.5rem] border-border bg-card shadow-sm overflow-hidden relative">
                        ...
                        <PropertyDocumentUploadZone
                            propertyId={propertyId}
                            initialFiles={watch("documents") as any}
                            onFilesChange={(files: any[]) => {
                                const mapped = files.map((f: any, idx: number) => ({
                                    ...f,
            ...
            {/* Sticky Actions */}
            <div className="fixed bottom-0 right-0 p-4 z-50 flex justify-end gap-2">
